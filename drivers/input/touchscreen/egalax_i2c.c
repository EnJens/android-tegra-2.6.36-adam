...
static ssize_t egalax_cdev_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
	...
	read_cnt = kfifo_out(cdev->pDataFiFo, fifo_read_buf, count);
...
static int egalax_i2c_measure(struct i2c_client *client)
{
	...
	{
		kfifo_in(p_char_dev->pDataFiFo, x_buf, count);
...
//	kfifo_init(pCharDev->pDataFiFo, pCharDev->pFiFoBuf, FIFO_SIZE);
