...
static int ecs_ctrl_ioctl(struct file *file, 
	unsigned int cmd, unsigned long arg);
static long ecs_ctrl_unlocked_ioctl(struct file *file, 
	unsigned int cmd, unsigned long arg);
...
static struct file_operations ecs_ctrl_fops = {
	...
	.unlocked_ioctl	= ecs_ctrl_unlocked_ioctl,
};
        ...
        unsigned int cmd, unsigned long arg)
{
        ...
        lock_kernel();
        ret = ecs_ctrl_ioctl(file, cmd, arg);
        unlock_kernel();
...
static int __init ecompass_init(void)
{
	...
	printk(KERN_DEBUG "ecompass driver: init\n");
