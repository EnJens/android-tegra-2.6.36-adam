...
static int harmony_wifi_status_register(void (*callback)(int , void *), void *);
static struct clk *wifi_32k_clk;

static int harmony_wifi_reset(int on);
static int harmony_wifi_power(int on);
static int harmony_wifi_set_carddetect(int val);

static struct wifi_platform_data harmony_wifi_control = {
        .set_power      = harmony_wifi_power,
        .set_reset      = harmony_wifi_reset,
        .set_carddetect = harmony_wifi_set_carddetect,
};

static struct platform_device harmony_wifi_device = {
        .name           = "bcm4329_wlan",
        .id             = 1,
        .dev            = {
                .platform_data = &harmony_wifi_control,
        },
};
...
static struct tegra_sdhci_platform_data tegra_sdhci_platform_data1 = {
        .clk_id = NULL,
        .force_hs = 0,
        .register_status_notify = harmony_wifi_status_register,
        .cccr   = {
                .sdio_vsn       = 2,
                .multi_block    = 1,
                .low_speed      = 0,
...
static int harmony_wifi_status_register(
...
static int __init harmony_wifi_init(void)
{
        ...
        gpio_direction_output(HARMONY_WLAN_PWR, 0);
        gpio_direction_output(HARMONY_WLAN_RST, 0);

        platform_device_register(&harmony_wifi_device);

        device_init_wakeup(&harmony_wifi_device.dev, 1);
        device_set_wakeup_enable(&harmony_wifi_device.dev, 0);
